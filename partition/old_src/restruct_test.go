...
func TestRestructHPE(t *testing.T) {
	data, err := os.ReadFile("./out/la-Health and Physical Education.json") // la-Health and Physical Education.json // HPE.json
	if err != nil {
		log.Fatalln(err)
	}
	js := reStructHPE(string(data))
	os.WriteFile("./out/la-Health and Physical Education(restruct).json", []byte(js), os.ModePerm)
}
